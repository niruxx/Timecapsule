...
// Used only for a Wayback Machine import (see importWaybackSnapshot below): the raw HTML fetched
// from archive.org has no document URL of its own, so a <base> tag is injected before handing it
// to page.setContent() - that's what lets every relative image/stylesheet/link in that HTML
// resolve against the *original* site instead of about:blank, the same way a normal page.goto()
// would resolve them against wherever it actually navigated.
function injectBaseTag(html, targetUrl) {
  const baseTag = `<base href="${targetUrl.replace(/"/g, '&quot;')}">`;
  return /<head[^>]*>/i.test(html) ? html.replace(/<head[^>]*>/i, (match) => `${match}${baseTag}`) : `${baseTag}${html}`;
}
...
function findMediaCandidates() {
  ...
  document.querySelectorAll('video[src], audio[src], video source[src], audio source[src]').forEach((el) => {
    try {
      urls.add(new URL(el.getAttribute('src'), document.baseURI).href);
    } catch {
...
function inlineDom(resourceMap, cssMap) {
  // document.baseURI (not location.href) - the two only diverge when a page carries its own
  // <base> tag, which is exactly what a Wayback Machine import injects (see injectBaseTag above)
  // since page.setContent() never touches location.href at all, leaving it stuck on about:blank.
  // Resolving against that would silently break every relative/protocol-relative URL in the page.
  const abs = (url) => {
    try {
...
async function saveArchive(page, targetUrl, dir, baseDir, options = {}) {
  ...
  emitStage('navigate');
  if (options.presetHtml) {
    // Wayback Machine import (see importWaybackSnapshot below): render already-fetched HTML
    // instead of navigating live. No bot-check hand-off here - a static snapshot can't present one.
    await page.setContent(injectBaseTag(options.presetHtml, targetUrl), { waitUntil: 'networkidle2', timeout: NAV_TIMEOUT_MS });
  } else {
    await page.goto(targetUrl, { waitUntil: 'networkidle2', timeout: NAV_TIMEOUT_MS });
  ...
  const capturedAt = options.presetCapturedAt || new Date().toISOString();
  ...
  if (article && article.textContent.trim().length >= MIN_ARTICLE_TEXT_LENGTH) {
    ...
    if (options.isMainPage && !options.skipChangeDetection) {
      try {
...
// Imports a single already-fetched Wayback Machine snapshot by running it through the exact same
// saveArchive() pipeline as a live capture - just with page.setContent(html) standing in for
// page.goto(url). That's what gets an imported snapshot the same PDF/screenshot/WARC/article/AI
// treatment as anything archived live, rather than just dropping in a bare page.html. Historical
// note: the dir/timestamp reflect when the snapshot was actually taken on the web (`capturedAt`),
// not when it was imported into TimeCapsule.
async function importWaybackSnapshot(browser, targetUrl, html, capturedAt, baseDir, options = {}) {
  const job = options.job || null;
  const onProgress = options.onProgress || (() => {});
  ...
  if (job) job.page = page;
  try {
    const dir = buildMainDir(baseDir, targetUrl, formatTimestamp(new Date(capturedAt)));
    if (job) job.mainDir = dir;

    const result = await saveArchive(page, targetUrl, dir, baseDir, {
      userAgent: options.userAgent,
      saveImages: options.saveImages,
      saveMedia: options.saveMedia,
      job,
      isMainPage: true,
      presetHtml: html,
      presetCapturedAt: capturedAt,
      skipChangeDetection: true,
      source: 'wayback',
      onStage: (pagePercent, label) => onProgress({ type: 'stage', percent: pagePercent, label, url: targetUrl }),
    });
    ...
    onProgress({ type: 'page', role: 'main', url: targetUrl });
    onProgress({ type: 'stage', percent: 100, label: 'Done', url: targetUrl });
    return toRelative(baseDir, result);
  } finally {
    if (job) job.page = null;
    await page.close().catch(() => {});
  }
